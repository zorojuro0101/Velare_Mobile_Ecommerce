...
def get_riders():
    """Fetch all riders with filtering using Supabase"""
    try:
        print("\n" + "="*80)
        print("🏍️ [ADMIN RIDERS] Fetching riders list...")
        print("="*80 + "\n")
        ...
        try:
            # Build query for riders with user info
            rider_query = supabase.table('riders').select('''
                rider_id,
                user_id,
                first_name,
                last_name,
            ...
            if status_filter != 'all':
                rider_query = rider_query.eq('users.status', status_filter)
            
            # Order by rider_id instead of users.created_at (can't order by foreign table columns directly)
            rider_query = rider_query.order('rider_id', desc=True)
            riders_response = rider_query.execute()
            ...
            # Fetch addresses for all riders
            addresses_dict = {}
            if rider_ids:
                addresses_response = supabase.table('addresses').select('*').eq('user_type', 'rider').in_('user_ref_id', rider_ids).execute()
            ...
            for rider in riders:
                ...
                rider_data = {
                    ...
                    'id_type': rider.get('id_type'),
                    'id_file_path': rider.get('id_file_path'),
                ...
                if search_query:
                    ...
                    if (search_lower in (rider_data['first_name'] or '').lower() or
                        search_lower in (rider_data['last_name'] or '').lower() or
                        search_lower in (rider_data['email'] or '').lower()):
                        filtered_riders.append(rider_data)
                else:
                    filtered_riders.append(rider_data)
            
            # Count by status - optimize by counting from the fetched data instead of making 5 separate queries
            print("🔍 Counting riders by status...")
            
            # Get all riders with their status
            all_riders_response = supabase.table('users').select('user_id, status').eq('user_type', 'rider').execute()
            all_riders_data = all_riders_response.data if all_riders_response.data else []
            
            # Count by status in Python instead of making multiple DB queries
            pending_count = sum(1 for r in all_riders_data if r.get('status') == 'pending')
            active_count = sum(1 for r in all_riders_data if r.get('status') == 'active')
            rejected_count = sum(1 for r in all_riders_data if r.get('status') == 'rejected')
            suspended_count = sum(1 for r in all_riders_data if r.get('status') == 'suspended')
            banned_count = sum(1 for r in all_riders_data if r.get('status') == 'banned')
...
def approve_rider(rider_id):
    """Approve a rider account using Supabase"""
    try:
        print(f"\n✅ [APPROVE RIDER] Processing rider_id: {rider_id}")
...
def reject_rider(rider_id):
    """Reject a rider account using Supabase"""
    try:
        print(f"\n❌ [REJECT RIDER] Processing rider_id: {rider_id}")
...
def suspend_rider(rider_id):
    """Suspend a rider account using Supabase"""
    try:
        print(f"\n⏸️ [SUSPEND RIDER] Processing rider_id: {rider_id}")
        ...
        try:
            # Get rider's user_id and details
            rider_response = supabase.table('riders').select('user_id, first_name, last_name, users(email)').eq('rider_id', rider_id).execute()
            ...
            print(f"✅ Rider {rider_id} suspended")
...
def ban_rider(rider_id):
    """Ban a rider account using Supabase"""
    try:
        print(f"\n🚫 [BAN RIDER] Processing rider_id: {rider_id}")
