...
def get_user_notifications(user_id):
    ...
    try:
        # Fetch notifications with delivery status
        ...
        for notification in notifications_response.data:
            # Get delivery status if order_id exists
            delivery_status = None
            if notification.get('order_id'):
                delivery_response = supabase.table('deliveries').select('status').eq('order_id', notification['order_id']).execute()
