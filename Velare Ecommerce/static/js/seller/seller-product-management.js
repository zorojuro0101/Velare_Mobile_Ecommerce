document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAndDisplayOrders() {
        ...
        try {
            ...
            res.orders.forEach(order => {
                ...
                const productImage = item.image ? `/${item.image}` : '/static/images/image.png';
                        ...
                        <div class="product-dropdown-container">
                            ...
                            <div class="product-dropdown-list" id="dropdown-${order.order_id}" style="display: none;">
                                ${order.items.slice(1).map(it => {
                                    const itemImage = it.image ? `/${it.image}` : '/static/images/image.png';
    ...
    async function fetchAndDisplayProducts() {
        try {
            ...
            const response = await fetch(`/api/products/list?_t=${timestamp}`, {
                method: 'GET',
                cache: 'no-store',
                headers: {
            ...
            filteredProducts.forEach(product => {
                const imageUrl = product.primary_image || '/static/images/user.png';
                        ...
                        if (imagesToDisplay && imagesToDisplay.length > 0) {
                            ...
                            variantImagesHTML = `
                                <div class="variant-images-preview">
                                    <img src="/${firstImage.url}" alt="${variant.color_name || variant.color}" 
                            ...
                            variantImagesHTML = `
                                <img src="/${variantImageUrl}" alt="${variant.color_name || variant.color}" 
                                     class="variant-image-thumb clickable-variant-image"
                                     data-variant-id="${variant.variant_id}"
                                     data-product-id="${product.product_id}"
                                     title="Click to view">
                ...
                img.addEventListener('click', function(event) {
                    ...
                    if (product && product.variants && product.variants.length > 0) {
                        ...
                        if (uniqueImages.length > 0) {
                            showImageCarouselModal(uniqueImages.map(url => '/' + url), 0);
                        } else {
                ...
                img.addEventListener('click', function(event) {
                    ...
                    if (product && product.variants) {
                        ...
                        if (imagesToShow && imagesToShow.length > 0) {
                            ...
                            const variantImages = imagesToShow.map(img => '/' + img.url);
    ...
    function displayEditCurrentVariants(variants) {
        ...
        Object.entries(colorGroups).forEach(([colorHex, colorGroup]) => {
            ...
            if (firstVariant.images && firstVariant.images.length > 0) {
                ...
                window.variantImagesData[imageKeyId].push({
                    url: firstVariant.image_url,
                    displayUrl: `/${firstVariant.image_url}`,
                    isExisting: true,
                    isPrimary: true
                });
            } else {
    ...
    if (editProductForm) {
        editProductForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Wait for database commit to complete before refreshing
                await new Promise(resolve => setTimeout(resolve, 500));
                
                console.log('🔄 Refreshing product list...');
                
                // Refresh product list with updated data
                await fetchAndDisplayProducts();
