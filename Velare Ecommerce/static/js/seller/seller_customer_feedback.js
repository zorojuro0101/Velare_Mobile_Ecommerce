...
function applyFilters() {
	...
	filteredData = feedbackData.filter(feedback => {
		...
		let dateMatch = true;
		if (dateFrom || dateTo) {
			const reviewDate = new Date(feedback.created_at);
			...
			if (dateFrom && dateTo) {
				...
				// Only from date - show reviews on or after this date
				const fromDate = new Date(dateFrom);
				dateMatch = reviewDate >= fromDate;
			} else if (dateTo) {
				// Only to date - show reviews on or before this date
				const toDate = new Date(dateTo);
				toDate.setHours(23, 59, 59, 999);
				dateMatch = reviewDate <= toDate;
			}
		}
		...
		return dateMatch && ratingMatch && searchMatch;
	});
