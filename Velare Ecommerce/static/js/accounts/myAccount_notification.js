...
// Auto-refresh notifications from server
async function refreshNotifications() {
    try {
        ...
        // Parse the new HTML to extract notifications
        const parser = new DOMParser();
        const newDoc = parser.parseFromString(html, 'text/html');
        const newNotificationsContent = newDoc.querySelector('.notifications-content');
        
        if (!newNotificationsContent) return;
        
        const currentNotificationsContent = document.querySelector('.notifications-content');
        if (!currentNotificationsContent) return;
        
        // Get current notification IDs
        const currentIds = new Set(
            Array.from(currentNotificationsContent.querySelectorAll('.notification-item'))
                .map(item => item.getAttribute('data-notification-id'))
        );
        
        // Get new notification IDs
        const newIds = new Set(
            Array.from(newNotificationsContent.querySelectorAll('.notification-item'))
                .map(item => item.getAttribute('data-notification-id'))
        );
        ...
        // If there are new notifications, refresh the page
        if (hasNewNotifications) {
            console.log('[NOTIFICATIONS] New notifications detected, refreshing...');
            currentNotificationsContent.innerHTML = newNotificationsContent.innerHTML;
            
            // Re-initialize handlers for new notifications
            initializeNotificationHandlers();
