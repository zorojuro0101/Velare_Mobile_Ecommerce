...
function createSellerCard(seller) {
    ...
    const statusClass = seller.status.toLowerCase();
    const statusLabel = seller.status.charAt(0).toUpperCase() + seller.status.slice(1);
    
    return `
        <div class="approval-card" data-seller-id="${seller.account_id}" data-user-id="${seller.user_id}" data-status="${seller.status}">
            <div class="card-body">
                    ...
                    <div class="buyer-name-header">
                        ${seller.shop_logo ? 
                            `<div class="small-avatar" style="background: none; border: 2px solid rgba(211, 189, 155, 0.3); padding: 0; overflow: hidden;">
                                <img src="${seller.shop_logo.startsWith('/') ? seller.shop_logo : '/static/' + seller.shop_logo}" alt="Shop Logo" style="width: 100%; height: 100%; object-fit: cover;" onerror="console.error('Failed to load shop logo:', this.src); this.parentElement.innerHTML='<div style=\\'width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#d3bd9b 0%,#c4a882 100%);color:#fff;font-family:Playfair Display,serif;font-size:1.1rem;font-weight:700;\\'>${initials}</div>';">
                            </div>` :
