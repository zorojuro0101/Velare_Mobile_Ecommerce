...
function createRiderCard(rider) {
    ...
    const statusClass = rider.status.toLowerCase();
    const statusLabel = rider.status.charAt(0).toUpperCase() + rider.status.slice(1);
    
    return `
        <div class="approval-card" data-rider-id="${rider.account_id}" data-status="${rider.status}">
            <div class="card-body">
                    ...
                    <div class="buyer-name-header">
                        ${rider.profile_image ? 
                            `<div class="small-avatar" style="background: none; border: 2px solid rgba(211, 189, 155, 0.3); padding: 0; overflow: hidden;">
                                <img src="${rider.profile_image.startsWith('/') ? rider.profile_image : '/static/' + rider.profile_image}" alt="Profile" style="width: 100%; height: 100%; object-fit: cover;" onerror="console.error('Failed to load profile:', this.src); this.parentElement.innerHTML='<div style=\\'width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#d3bd9b 0%,#c4a882 100%);color:#fff;font-family:Playfair Display,serif;font-size:1.1rem;font-weight:700;\\'>${initials}</div>';">
                            </div>` :
