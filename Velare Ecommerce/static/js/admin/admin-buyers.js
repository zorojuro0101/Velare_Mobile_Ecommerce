...
function createBuyerCard(buyer) {
    ...
    const statusClass = buyer.status.toLowerCase();
    const statusLabel = buyer.status.charAt(0).toUpperCase() + buyer.status.slice(1);
    
    const cardHtml = `
        <div class="approval-card user-card" data-buyer-id="${buyer.account_id}" data-user-id="${buyer.user_id}" data-status="${buyer.status}">
            <div class="card-body">
